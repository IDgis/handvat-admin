...
Template.couplinglist.helpers({
  ...
  couplingontwerp: function() {
    ...
    CouplingOntwerp.find().forEach(function(newOntwerp) {
      ...
      var ops = [];
      newOntwerp.ontwerpprincipe.forEach(function(op) {
  ...
  couplingleidend: function() {
    ...
    CouplingLeidend.find().forEach(function(newLeidend) {
      newLeidend.landschapstype = Text.findOne( { "_id": newLeidend.landschapstype } );
      
      var lbs = [];
      newLeidend.leidend_beginsel.forEach(function(lb) {
